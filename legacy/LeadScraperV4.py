...
class LeadScraper():
    ...
    async def handler(self):
        ...
        async with async_playwright() as playwright:  
            ...
            await asyncio.gather(*[self.fetch_search_results(await browser.new_context(user_agent=agent.random,viewport={'width':600,'height':700})) for i in range(0,self.pm+2)])
    ...
    async def fetch_search_results(self,context):
        ...
        while True:
            if not self.query_tasks.empty():      
              ...
              while self.count<self.min and uid in self.files and self.flg<self.pm:
               if not tries:
               ...
               counter += 1
              self.flg+=1
              if(uid in self.files and self.flg<self.pm):
